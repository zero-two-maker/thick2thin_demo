...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        x = rearrange(x, 'b (d h w) c -> b c d h w', h=h, w=w).contiguous()
        return x
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True, apply_att=False,
        ...
        if apply_att:
            self.att = BasicTransformerBlock(out_channels, 8, 16, context_dim=out_channels)
        else:
            self.att = None


    def forward(self, x, f_age):
