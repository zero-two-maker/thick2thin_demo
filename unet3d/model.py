...
class Age_adapter(nn.Module):
    def __init__(self, time_type, channel):
        super().__init__()
        self.age_tensors = nn.Parameter(torch.randn([time_type, 512, 1]))
        self.condation_out = torch.nn.Linear(1, channel)
        self.norm = nn.LayerNorm(channel)

    def forward(self, age):
        cc = self.age_tensors.index_select(0, age)
        cc = self.norm(self.condation_out(cc))
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.age_adapter = Age_adapter(7, f_maps[-1])
        # create encoder path
