...
class UNetTrainer:
    ...
    def train(self):
        ...
        # for t in range(5):
            ...
            input, target, age = self._split_training_batch(t)
            # input = torch.randn(1, 1, 192, 224, 192).to("cuda:0")
            # target = torch.randn(1, 4, 192, 224, 192).to("cuda:0")
            # weight = None
            # age = torch.tensor([0]).to("cuda:0")
            output, loss = self._forward_pass(input, target, age)
    ...
    def _split_training_batch(self, t):
        ...
        t = _move_to_gpu(t)
        input, target, age = t
        return input, target, age

    def _forward_pass(self, input, target, age, weight=None):
    ...
    def _save_checkpoint(self, is_best):
        ...
        }, is_best, self.num_epochs, checkpoint_dir=self.checkpoint_dir)
